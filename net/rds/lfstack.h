...
static inline void lfstack_push(struct lfstack *stack, struct lfstack_el *el)
{
	...
	while (true) {
		...
		if (try_cmpxchg128((u128 *)&stack->first, &prev.val, new.val))
			break;
	}
#else
...
static inline void lfstack_push_many(struct lfstack *stack,
				     struct lfstack_el *el_first,
				     struct lfstack_el *el_last)
{
#ifdef LFSTACK_LOCKFREE
	union lfstack_header prev, new;
...
#else
	llist_add_batch((struct llist_node *)el_first, (struct llist_node *)el_last,
			(struct llist_head *)stack);
#endif
}
