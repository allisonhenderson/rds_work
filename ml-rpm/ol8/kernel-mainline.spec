...
Source47: modules-aarch64-emb3.yaml.S
...
Conflicts: shim-aa64 < 15.8-4.0.2\
...
%endif
    ...
    if [[ "$Flavour" == "emb3" || "$Flavour" == "emb3debug" ]]; then
        modules_file=modules-aarch64-emb3
    fi
    ...
    (cd $RPM_BUILD_ROOT && $RPM_SOURCE_DIR/filter-modules.py \
        ...
        -D"Arch_%{_target_cpu}" \
        -D"Flavour_${Flavour}" \
        $RPM_SOURCE_DIR/${modules_file}.yaml.S \
